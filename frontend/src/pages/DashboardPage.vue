...
<template>
  <PageContainer>
    ...
    <div class="grid grid-cols-[repeat(auto-fill,minmax(220px,1fr))] gap-4">
      <StatCard :label="t('dashboard.availableModules')" :value="modules.allowed.length" icon="layout-grid" />
      <StatCard
        :label="t('dashboard.permissions')"
        :value="auth.isAdmin ? '∞' : auth.permissions.length"
        icon="shield"
      />
      <StatCard :label="t('dashboard.profile')" :value="profileLabel" icon="user" />
    </div>

    <div class="mt-8">
      <h2 class="mb-4 text-base font-semibold tracking-tight">{{ t('dashboard.quickAccess') }}</h2>
      <div v-if="modules.allowed.length" class="grid grid-cols-[repeat(auto-fill,minmax(220px,1fr))] gap-4">
        <ModuleCard
      ...
      <EmptyState v-else icon="layout-grid" :title="t('dashboard.noModules')" />
    </div>
  </PageContainer>
</template>
