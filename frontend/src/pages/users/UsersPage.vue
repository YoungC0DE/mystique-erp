...
<template>
  <PageContainer>
    ...
    <Table>
      ...
      <TableBody>
        <TableRow v-for="u in users" :key="u.id">
          ...
          <TableCell>
            <Badge v-for="r in u.roles ?? []" :key="r.id" variant="muted" class="mr-1">
              {{ r.name }}
            </Badge>
    ...
    <BaseModal v-if="showModal" :title="editing ? t('users.editUser') : t('users.newUser')" @close="showModal = false">
