...
<template>
  <PageContainer>
    ...
    <Table>
      ...
      <TableBody>
        <TableRow v-for="r in roles" :key="r.id">
          ...
          <TableCell>
            <Badge v-for="p in r.permissions ?? []" :key="p.id" variant="primary" class="mr-1">
              {{ p.name }}
            </Badge>
    ...
    <BaseModal v-if="showModal" :title="editing ? t('roles.editRole') : t('roles.newRole')" @close="showModal = false">
      ...
      <div class="space-y-1.5">
        ...
        <div class="grid grid-cols-2 gap-2 rounded-md border border-border p-3">
          <label v-for="p in permissions" :key="p.id" class="flex cursor-pointer items-center gap-2 text-sm">
            <Checkbox
