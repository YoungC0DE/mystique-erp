...
const moduleFields = computed(() =>
  (selectedModule.value?.fields ?? []).slice().sort((a, b) => a.order - b.order),
)
...
async function load(): Promise<void> {
  try {
    const [reportsRes, modulesRes] = await Promise.all([
      reportsService.list(page.value),
      modulesService.list(1, 100),
    ])
...
<template>
  <PageContainer>
    ...
    >
      <div class="space-y-4">
        ...
        <div class="space-y-1.5">
          ...
          <select v-model="form.module_id" class="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm">
        ...
        <div v-if="form.field_keys.length">
          ...
          <div class="max-h-40 space-y-1 overflow-y-auto rounded-lg border border-border/80 p-2">
            <div v-for="(key, i) in form.field_keys" :key="key" class="flex items-center gap-2 rounded border px-2 py-1 text-sm">
              ...
              <Button variant="ghost" size="icon" class="h-7 w-7" :disabled="i === form.field_keys.length - 1" @click="moveField(i, 1)">
        ...
        <div class="border-t pt-4">
          <div class="mb-3 flex items-center justify-between gap-2">
            ...
            <Button
              v-if="form.filtersDraft.length"
              variant="ghost"
              size="sm"
              @click="form.filtersDraft = []"
            >
              {{ t('filters.clear') }}
            </Button>
          </div>
    ...
    <BaseModal
      v-if="showRunModal && runningReport"
      large
