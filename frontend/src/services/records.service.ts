...
export const recordsService = {
  ...
  ): Promise<Paginated<ModuleRecord>> {
  ...
  async moveIntegrated(moduleId: string, externalId: string, status: string): Promise<ModuleRecord> {
    const { data } = await http.post<{ data: ModuleRecord }>(`/modules/${moduleId}/records/${externalId}/move`, {
      status,
    });
    return data.data;
  },
  ...
  async getNote(moduleId: string, recordKey: string): Promise<RecordNote> {
    const { data } = await http.get<{ data: RecordNote }>(`/modules/${moduleId}/records/${recordKey}/note`);
    return data.data;
  },

  async saveNote(moduleId: string, recordKey: string, body: string | null): Promise<RecordNote> {
    const { data } = await http.put<{ data: RecordNote }>(`/modules/${moduleId}/records/${recordKey}/note`, { body });
    return data.data;
  },
};
