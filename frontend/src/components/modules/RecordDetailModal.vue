...
<template>
  <BaseModal large :title="title ?? t('recordDetail.title')" @close="emit('close')">
    ...
    <div class="space-y-4">
      <div v-for="(row, rowIndex) in layoutRows" :key="rowIndex" :class="['grid gap-3', rowGridClass(row.field_keys.length)]">
        <div
          v-for="key in row.field_keys"
          :key="key"
    ...
    <div class="mt-6 border-t pt-5">
      ...
      <Textarea
        v-model="noteBody"
        :placeholder="t('recordDetail.internalNotePlaceholder')"
        rows="4"
      />
