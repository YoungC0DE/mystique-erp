  ...
  <div class="flex min-h-screen bg-background">
    ...
    >
      <div class="flex items-center gap-3 border-sidebar-border px-5 py-5">
        ...
        <div class="min-w-0">
          <span class="block truncate text-[15px] font-semibold tracking-tight text-sidebar-foreground">
            Mystique CRM
          </span>
    ...
    <div class="flex min-w-0 flex-1 flex-col">
      ...
      >
        <Button variant="ghost" size="icon" class="hidden max-[860px]:inline-flex" @click="sidebarOpen = !sidebarOpen">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger as-child>
            <button
              type="button"
              class="flex cursor-pointer items-center gap-2.5 rounded-lg bg-background px-2 py-1.5 text-left transition-all duration-200 hover:bg-accent/50"
              :aria-label="t('userMenu.openMenu')"
            >
              <span class="grid h-8 w-8 place-items-center rounded-lg bg-primary/10 text-xs font-semibold text-primary">
                {{ initials }}
              </span>
